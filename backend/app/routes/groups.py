...
def create_group(page_id: int, payload: GroupCreate, user: dict = Depends(require_user)):
    with get_db_connection() as conn:
        ...
        return {
            ...
            "bg_color": g["bg_color"], "header_bg_color": g["header_bg_color"], "header_text_color": g["header_text_color"], "bookmark_title_color": g["bookmark_title_color"], "icon_color": g["icon_color"],
...
def update_group(group_id: int, payload: GroupUpdate, user: dict = Depends(require_user)):
    with get_db_connection() as conn:
        ...
        icon_color = group["icon_color"] if payload.icon_color is None else (payload.icon_color or None)
        ...
        conn.execute(
            """
            UPDATE groups SET page_id=?, title=?, icon_url=?, bg_color=?, header_bg_color=?, header_text_color=?, bookmark_title_color=?, icon_color=?, transparency=?, display_mode=?,
                icon_size=?, bookmark_align=?, visible_limit=?, bookmark_sort=?, col=?, position=?, manual_x=?, manual_y=?, manual_z=?, updated_at=? WHERE id=?
            """,
            (
                target_page_id, title, icon_url, bg_color, header_bg_color, header_text_color, bookmark_title_color, icon_color, transparency, display_mode,
...
def duplicate_group(group_id: int, user: dict = Depends(require_user)):
    with get_db_connection() as conn:
        ...
        cur = conn.execute(
            ...
            """,
            (
                group["page_id"], f'{group["title"]} Copy', group["icon_url"], group["bg_color"], group["header_bg_color"], group["header_text_color"], group["bookmark_title_color"], group["icon_color"] if "icon_color" in group.keys() else None, group["transparency"],
        ...
        for bookmark in conn.execute("SELECT * FROM bookmarks WHERE group_id = ? ORDER BY position, id", (group_id,)).fetchall():
            conn.execute(
                ...
                (
                    ...
                    None, None, bookmark["docker_ref"], bookmark["title_color"] if "title_color" in bookmark.keys() else None, bookmark["icon_color"] if "icon_color" in bookmark.keys() else None, bookmark["position"], ts, ts,
                ),
            )
        conn.commit()
